...
def ask_groq_analysis(text):
    ...
    3. **中立與無效過濾**：若玩家僅為「單純分享數據/攻略」且語氣中立，或僅為無意義之「卡、推、路過」，請保留 JSON 格式但將內容留白，輸出如下：
       {{ "reviews": [{{ "main_category": "", "sub_category": "", "target_character": "null", "sentiment_score": 0, "reason": "", "keywords": [] }}] }}

    【核心指令 2：語意證據與防污染 (防幻覺機制)】
    1. **原文證據 (關鍵)**：reason 必須包含原文引用，格式如：『回覆者提到的「[原文短句]」顯示出...』。
    2. **寧缺勿濫原則**：若在資料中「找不到具體的評價原因」或「無法提取有意義的原文短句」，絕對禁止硬湊字詞或拿 keywords 造句。遇此情況請一律輸出上述的留白格式。
    3. **欄位脫鉤**：`keywords` 是用於文字雲統計的名詞/形容詞，`reason` 是分析邏輯。嚴禁在 `reason` 中單純把 `keywords` 串接起來敷衍了事。
    4. **嚴禁抄襲範例**：reason 與 keywords 必須 100% 取自【輸入資料】。若資料中未出現該字眼，絕對禁止輸出。
    5. **遊戲術語識別**：
    ...
    【嚴格輸出格式】
    請直接輸出 JSON 物件，不要加上任何 Markdown 標記 (如 ```json)。請務必遵守以下資料型態與結構，絕對不要照抄括號內的佔位符字眼：
    {{
    "reviews": [
        {{
        "main_category": "<填入判定出的主分類，若無則留白>",
        "sub_category": "<填入判定出的次分類，若無則留白>",
        "target_character": "<填入目標角色名稱，若無則填 null>",
        "sentiment_score": <填入 -5 到 5 的整數，絕對中立填 0>,
        "reason": "<填入分析理由，必須包含【輸入資料】中的原文引用>",
        "keywords": ["<萃取出的關鍵字1>", "<萃取出的關鍵字2>"]
        }}
    ]
    }}
    """
