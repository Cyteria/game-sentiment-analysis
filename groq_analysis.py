...
def ask_groq_analysis(text):
    ...
    2. **攻略與心得區分**：若玩家只是在「單純分享隊伍配置/數值」，且語氣中立，請交白卷 `{{"reviews": []}}`。只有當玩家對遊戲機制有明顯的「爽/讚/強/超模/可愛」或「爛/燙/難/制裁」等情緒時才進行分析。

    【核心指令 2：嚴禁抄襲範例 (防污染)】
    1. **禁止複製範例字眼**：輸出結果中的 `reason` 與 `keywords` 必須 100% 取自【輸入資料】。若資料中未出現「保底」、「閃退」等字眼，結果絕對不准出現。
    ...
    - 5 到 1：正面情緒（例如：可愛、滿意、神作、超強、還原）。情緒越強烈分數越高。
    - 0：絕對中立（無任何情緒起伏的客觀陳述）。⚠️警告：只要你的 reason 裡出現了「喜歡、可愛、滿意、熱情」等正面詞彙，分數【絕對不能是 0】！至少要是 1 以上。
    - -1 到 -5：負面情緒（例如：爛、Bug多、弱、退坑、炎上）。情緒越強烈分數越低。

    【嚴格輸出格式 (JSON 範例)】
    請嚴格依照以下 JSON 格式輸出，不要加上任何 Markdown 標記 (如 ```json)：
    {{
    "reviews": [
        {{
        "main_category": "角色",
        "sub_category": "立繪",
        "target_character": "卡提希婭",  // 若無特定角色請填 null (不要加引號)
        "sentiment_score": 4,            // 必須是整數 (Integer)，絕不能是字串
        "reason": "有回覆者提到卡提希婭的模型非常可愛，玩家情緒非常正面。",
        "keywords": ["模型", "可愛", "正面"] // 必須是字串陣列 (Array of Strings)
        }}
    ]
    }}
    """
